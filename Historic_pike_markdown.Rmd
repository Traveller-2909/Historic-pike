...
=======
  transmute(Fish_ID, Date, Waterbody, Total_Length_mm, Sex, Weight, Oldenburg_ID, Genotype)
>>>>>>> b2052a59afeb74cb36b788577d108f9727d3c070
...
flextable(SizeByDecade_wide) %>%
  ...
  autofit()
=======
  reframe(Length_LI_mean = mean(Length_LI),
          SDlength = sd(Length_LI))
>>>>>>> b2052a59afeb74cb36b788577d108f9727d3c070
...
```{r claculate size-independent growth index, include=FALSE, eval=FALSE}
>>>>>>> b2052a59afeb74cb36b788577d108f9727d3c070
rm(list = ls())
...
length(unique(Sexpike2$Oldenburg_ID))
...
Growthdata <- Pikemaster4 %>%
  ...
  mutate(GIndex = case_when(is.na(GIndex) == "TRUE" ~ Length_back,
                            TRUE ~ GIndex)) %>%
  transmute(Fish_ID, Sample, Lifeyear, Cohort, Year, Sex, 
            Area = case_when(Area %in% c("KB", "SB", "VB") ~ "WRB",
                             Area %in% c("BEZ", "WB", "BEG", "GJB") ~ "NRB",
                             TRUE ~ Area), 
            Pisc_mm, Genotype, GIndex) %>%
  drop_na() %>%
  distinct()

fwrite(Growthdata, "Data/Pike/Pikemaster_modelling.csv")
>>>>>>> b2052a59afeb74cb36b788577d108f9727d3c070
```
...
=======
Growthdata <- fread("Data/Pike/Pikemaster_modelling.csv")
LUNGdata <- fread("Data/Environment/Lungdata-monthly-means.csv")
GDDdata <- fread("Data/Environment/GDD_Bodden.csv")
GDDdata <- GDDdata %>% transmute(Year = year,
                                 GDD)
>>>>>>> b2052a59afeb74cb36b788577d108f9727d3c070
...
standata_Growth = list(
  N = nrow(Growthdata),
  N_ind = length(unique(Growthdata$Fish_ID)),
  N_cohort = length(unique(Growthdata$Cohort)),
  N_year = length(unique(Growthdata$Year)),
  ind = as.integer(factor(Growthdata$Fish_ID)), #alt: mean^2/variance & variance/mean
  cohort = as.integer(factor(Growthdata$Cohort)),
  year = as.integer(factor(Growthdata$Year)),
  lifeyear = as.integer(factor(Growthdata$Lifeyear)),
  sex = as.integer(factor(Growthdata$Sex)),  
  growth_index = Growthdata$GIndex
)

# Model
stancode <- "Data/Pike/Stan/LinearMixedEffect.stan"

mod = stan_model(stancode, model_name = "age-specific pike growth")

#fitting
fit_1 = sampling(mod, iter = 500, data = standata_Growth, thin = 1, control = list(adapt_delta = 0.95, max_treedepth = 14))

print(fit_1, pars = c("alpha", "beta_lifeyear", "beta_sex", "sigma"))

fit2 <- brm(GIndex ~ Lifeyear + Sex + (1|Year) + (1|Cohort) + (1|Fish_ID), data = Growthdata, family = gaussian())
summary(fit2)
```
